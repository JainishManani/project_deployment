document.addEventListener("DOMContentLoaded", () => {
  fetch(`header.html?nocache=${Date.now()}`)
    ...
    .catch((error) => {
      ...
      document.getElementById("header").innerHTML =
        '<p class="text-danger text-center">Failed to load header</p>';
    });
  ...
  fetch(`footer.html?nocache=${Date.now()}`)
    ...
    .catch((error) => {
      ...
      document.getElementById("footer").innerHTML =
        '<p class="text-danger text-center">Failed to load footer</p>';
    });
      ...
      try {
        ...
        if (!response.ok) {
          ...
          successDiv.innerText =
            data.message || "Signup successful! Check email.";
  ...
  if (
    ...
    window.location.pathname.includes("books.html") ||
      ...
      fetch("/api/userbooks", {
        ...
        .then((data) => {
          ...
          if (bookList) {
            if (data.length === 0) {
              bookList.innerHTML =
                '<p class="text-center">No books added yet.</p>';
            } else {
                ...
                <div class="col-md-4 book-card">
                  <div class="card">
                    <div class="card-body">
                      ${
                        book.CoverURL
                          ? `<img src="${book.CoverURL}" alt="${book.Title} cover" class="img-fluid">`
                          : ""
                      }
                      <div>
                        ...
                        <button class="btn btn-secondary edit-book" data-id="${
                          book.UserBookID
                        }" aria-label="Edit book ${book.Title}">Edit</button>
                        <button class="btn btn-danger delete-book" data-id="${
                          book.UserBookID
                        }" aria-label="Delete book ${
                    book.Title
                  }">Delete</button>
                        <button class="btn btn-warning favorite-book" data-id="${
                          book.BookID
                        }" aria-label="Toggle favorite ${book.Title}">${
                    book.isFavorite ? "Unfavorite" : "Favorite"
                  }</button>
                        <button class="btn btn-info set-reminder" data-id="${
                          book.BookID
          ...
          if (remindersList) {
            if (data.length === 0) {
              remindersList.innerHTML =
                '<p class="text-center">No reminders set.</p>';
            } else {
              remindersList.innerHTML = data
                .map(
                  (reminder) => `
                <div class="col-md-4 book-card">
                  <div class="card">
                    <div class="card-body">
                      ...
                      <p class="card-text">Reminder Date: ${new Date(
                        reminder.ReminderDate
                      ).toLocaleString()}</p>
                      <p class="card-text">Note: ${
                        reminder.ReminderNote || "None"
                      }</p>
                      <button class="btn btn-danger delete-reminder" data-id="${
                        reminder.ReminderID
                ...
                <div class="col-md-6 mb-3">
                  <div class="card">
                    <div class="card-body">
                      ...
                      <p class="card-text">Posted: ${new Date(
                        review.DatePosted
                      ).toLocaleString()}</p>
                      <button class="btn btn-info view-comments" data-id="${
                        review.ReviewID
                      }" aria-label="View comments for ${
                    review.Username
                  }'s review">View Comments</button>
                    </div>
                  </div>
                </div>
              `
    ...
    if (window.location.pathname.includes("admin.html")) {
      ...
      // Load users + Add Role Change & Delete
        ...
        .then((data) => {
          if (usersList) {
            ...
            <tr>
              ...
              <td>${user.Role}</td>
              <td>
                <select class="form-select form-select-sm role-select" data-id="${
                  user.UserID
                }">
                  <option value="User"  ${
                ...
                <button class="btn btn-danger btn-sm delete-user ms-2" data-id="${
                  user.UserID
                }">
                  Delete
                </button>
         ...
         `
            ...
            document.querySelectorAll(".role-select").forEach((select) => {
              select.addEventListener("change", function () {
                fetch(
                  `/api/admin/users/${this.dataset.id}/role`,
                  {
                    method: "PATCH",
                    headers: {
                  ...
                  .then((r) =>
                    r.ok ? r.json() : Promise.reject("Failed to update role")
                  )
                  .then(() => {
                    successDiv.classList.remove("d-none");
                    successDiv.innerText = "Role updated successfully!";
                    setTimeout(() => successDiv.classList.add("d-none"), 3000);
                  })
                  .catch((err) => {
                    errorDiv.classList.remove("d-none");
                    errorDiv.innerText = err.message || "Failed to update role";
                  });
              });
            });
            ...
            document.querySelectorAll(".delete-user").forEach((btn) => {
              btn.addEventListener("click", function () {
                ...
                fetch(
                  `/api/admin/users/${this.dataset.id}`,
                  {
                    method: "DELETE",
                    headers: { Authorization: `Bearer ${token}` },
                  }
                  ...
                  .then((r) =>
                    r.ok ? r.json() : Promise.reject("Failed to delete user")
                  )
                  .then(() => {
                    this.closest("tr").remove();
                    successDiv.classList.remove("d-none");
                    successDiv.innerText = "User deleted!";
                    setTimeout(() => successDiv.classList.add("d-none"), 3000);
                  })
                  .catch((err) => {
                    errorDiv.classList.remove("d-none");
                    errorDiv.innerText = err.message || "Delete failed";
                  });
              });
            });
          }
    ...
    if (bookList && window.location.pathname.includes("books.html")) {
      bookList.addEventListener("click", async (e) => {
        ...
        if (e.target.classList.contains("edit-book")) {
          ...
          const readingStatus = prompt(
            "Enter new reading status (Read, Reading, To Read):"
          );
          ...
          if (
            !["Read", "Reading", "To Read"].includes(readingStatus) ||
            isNaN(progress) ||
            progress < 0 ||
            progress > 100
          ) {
          ...
          const reminderDate = prompt(
            "Enter reminder date (YYYY-MM-DD HH:MM:SS):"
          );
          const reminderNote = prompt(
            "Enter reminder note (optional, max 255 chars):"
          );
          if (
            !reminderDate ||
            !/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(reminderDate)
          ) {
            errorDiv.classList.remove("d-none");
          ...
          try {
            ...
            if (!response.ok) {
              ...
              const commentsHtml =
                ...
                <div class="card mb-2">
                  <div class="card-body">
                    <p class="card-text">${comment.Username}: ${
                          comment.ProgressPercent
                            ? `Progress: ${comment.ProgressPercent}%`
                            : ""
      ...
      reviewsList.addEventListener("submit", async (e) => {
        if (e.target.classList.contains("comment-form")) {
          ...
          const progressPercent = parseInt(
            document.getElementById(`progressPercent-${reviewId}`).value
          );
    ...
    if (searchForm) {
        ...
        try {
          ...
          if (searchResults) {
            if (data.books.length === 0) {
              ...
              searchResults.innerHTML = data.books
                .map(
                  (book) => `
                <div class="col-md-4 book-card">
                  <div class="card">
      ...
      if (searchQuery && autocompleteList) {
        searchQuery.addEventListener("input", async () => {
          ...
          try {
            const response = await fetch(
              `/api/books/autocomplete?query=${encodeURIComponent(
                query
              )}`,
              {
                // headers: { 'Authorization': `Bearer ${token}` }
