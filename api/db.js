...
async function getConnection() {
  // If we have a connection, try to keep it alive
  if (connection) {
    try {
  ...
  connection = await mysql.createConnection({
    ...
    port: process.env.MYSQL_PORT || 3306,
    user: process.env.MYSQL_USER,
    password: process.env.MYSQL_PASSWORD_DEPLOY,   // ← Unchanged
    database: process.env.MYSQL_DATABASE,
    ssl: {
      ca: fs.readFileSync(path.join(__dirname, '../ca.pem'), 'utf8'),
      rejectUnauthorized: false  // ← Unchanged
    },
    connectTimeout: 10000
  });

  console.log('New MySQL connection created (serverless-safe)');
  return connection;
}

// Helper function used in all routes
